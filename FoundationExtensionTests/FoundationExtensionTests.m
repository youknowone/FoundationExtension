...
- (void)testBase64String {
    ...
    XCTAssertEqual(encode.length, solution.length, @"length: %d expected: %d", encode.length, solution.length);
    XCTAssertTrue([encode isEqualToString:solution], @"result: %@", encode);

    NSData *decode = [NSData dataWithBase64String:solution];
    XCTAssertEqual(decode.length, data.length, @"length: %d expected: %d", decode.length, data.length);
